from src_gat_fusion2.base.base_train import BaseTrain
from tqdm import tqdm
import numpy as np
import tensorflow as tf
import time
from scipy.special import softmax
from src_gat_fusion2.utils.utils import auc_value, f1, cmatrix
import pickle as pkl

# import src_gat_fusion2.single_prediction.base.checkmate as checkmate
...
class GraphTrainer(BaseTrain):
    ...
    def train(self):
        ...
        optimal_eval_att_weights = []

        # traverse each epoch to train
        for cur_epoch in range(self.config.num_epochs):
            # print("█ Training on Epoch {}".format(cur_epoch))

            train_loss, eval_metric, eval_acc, test_metric, test_acc, eval_att_weights = self.train_epoch(prev_eval_metric, prev_test_metric)
            if cur_epoch % 20 == 0:
            ...
            if eval_metric > optimal_eval_metric:
                ...
                optimal_eval_att_weights = eval_att_weights
            # '''
        ...
        # save for generating contributions
        # att_weights_path = './optimal_eval_att_weights_supra.pkl'
        #
        # pkl.dump(optimal_eval_att_weights, open(att_weights_path, 'wb'))
    ...
    def train_epoch(self, prev_eval_metric, prev_test_metric):
        ...
        eval_loss, eval_acc, eval_metric, eval_att_weights = self.eval_step()
        ...
        return train_loss, eval_metric, eval_acc, test_metric, test_acc, eval_att_weights
    ...
    def train_step(self, prev_idx):
        ...
        feed_dict = {self.model.features: batch_x,
                     ...
                     self.model.bias_in: self.train_loader.biases_supra,
        ...
        _, loss, acc, probs, eval_metric, att_weights = self.sess.run([self.model.train_step,
                                             self.model.loss,
                                             self.model.accuracy,
                                             self.model.probs,
                                             self.model.f1,
                                             self.model.att_weights,
                                             ],
                                             feed_dict=feed_dict)
        return loss
    ...
    def eval_step(self):
        ...
        for idx in range(batches):
            ...
            loss, acc, preds, probs, f1, att_weights = self.sess.run([self.model.loss,
                                                             self.model.accuracy,
                                                             self.model.preds,
                                                             self.model.probs,
                                                             self.model.f1,
            ...
            eval_att_weights.extend(att_weights)
        ...
        return loss, acc, metric, eval_att_weights
    ...
    def test_step(self):
        ...
        for idx in range(batches):
            ...
            feed_dict = {self.model.features: batch_x,
                         self.model.labels: batch_y,
                         self.model.bias_in: self.train_loader.biases_supra,
                         # self.model.support: self.train_loader.support,
                         self.model.masks_in: batch_masks,
                         self.model.is_training: False,
                         self.model.attn_drop: 0.0,
                         self.model.ffd_drop: 0.0,
